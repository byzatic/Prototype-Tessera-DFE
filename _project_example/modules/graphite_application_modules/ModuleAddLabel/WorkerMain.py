...
class WorkerMain(i_Worker.WorkerMain):
    ...
    def execute(self) -> None:
        try:
            self.__logger.debug(f"Module ModuleAddLabel version {self.__module_version}")
            self.__logger.debug(f"START -> ModuleAddLabel")
            self.__ModuleProcess.run()
            self.__logger.debug(f"FINISH -> ModuleAddLabel")
