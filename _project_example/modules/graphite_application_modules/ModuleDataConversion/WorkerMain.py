...
class WorkerMain(i_Worker.WorkerMain):
    ...
    def execute(self) -> None:
        try:
            self.__logger.debug(f"Module ModuleDataConversion version {self.__module_version}")
            self.__logger.debug(f"START -> ModuleDataConversion")
            self.__ModuleProcess.run()
            self.__logger.debug(f"FINISH -> ModuleDataConversion")
