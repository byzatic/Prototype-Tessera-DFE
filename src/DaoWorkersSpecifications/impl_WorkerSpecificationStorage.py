...
class impl_WorkerSpecificationStorage(i_WorkerSpecificationStorage):
    ...
    def read(self, key: str) -> WorkerSpecification:
        ...
        @param key: string identification parameter
        @return: WorkerSpecification if success or raise LibByzaticCommon.Exceptions.OperationIncompleteException
        """
        try:
            ops_name = str(inspect.currentframe().f_code.co_name)
            self.__logger.debug(f"storage: {self.__storage_name} "
    ...
    def update(self, key: str, value: WorkerSpecification) -> int:
        ...
        try:
            ...
            if key in self.__storage:
                previous_value: WorkerSpecification = self.__storage[key]
    ...
    def __check_if_value_empty(self, value: WorkerSpecification, ops_name: str) -> None:
        try:
            pass
        except OperationIncompleteException as oie:
            raise OperationIncompleteException(oie.args, errno=oie.errno)
