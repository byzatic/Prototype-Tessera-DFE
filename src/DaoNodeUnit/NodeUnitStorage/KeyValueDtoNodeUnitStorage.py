...
class KeyValueDtoNodeUnitStorage(KeyValueCRUDInterface):
    def __init__(self, storage_name: str, critical_dump_flag: bool = True):
        ...
        """
        self.__logger: logging.Logger = logging.getLogger("DaoNodeUnit-NodeUnitStorage-logger")
        self.__storage: dict = {}
        self.__storage_name: str = storage_name
        self.__critical_dump_flag = critical_dump_flag

    def create(self, key: str, value: NodeUnit) -> None:
        """
    ...
    def read(self, key: str) -> NodeUnit:
        """
    ...
    def update(self, key: str, value: NodeUnit) -> int:
        """
        ...
        try:
            ops_name: str = str(inspect.currentframe().f_code.co_name)
            deep_value: NodeUnit = self.__local_deepcopy(value, ops_name)
            self.__logger.debug(f"storage: {self.__storage_name} "
                                f"operation: {ops_name} "
    ...
    def read_list_values(self) -> list[NodeUnit]:
        """
        ...
        try:
            ops_name: str = str(inspect.currentframe().f_code.co_name)
            list_item: list[NodeUnit] = []
            for item in self.__storage:
                list_item.append(item)
            storage_object: list[NodeUnit] = self.__local_deepcopy(list_item, ops_name)
            self.__logger.debug(f"storage: {self.__storage_name} "
                                f"operation: {ops_name} "
    ...
    def __local_deepcopy(self, value: Union[NodeUnit, list[NodeUnit]], ops_name: str) -> Union[NodeUnit, list[NodeUnit]]:
        try:
            self.__logger.debug(f"storage: {self.__storage_name} "
                                f"operation: {ops_name} "
    ...
    def __check_if_value_empty(self, value: NodeUnit, ops_name: str) -> None:
        try:
            if value is None:
                self.__logger.debug(f"storage: {self.__storage_name} "
                                    f"operation: {ops_name} "
    ...
    def get_storage_dto(self) -> DtoNodeUnitStorage:
        dto: DtoNodeUnitStorage = DtoNodeUnitStorage(
            local_storage=self.__storage
        )
        return dto

    def load_storage_dto(self, dto: DtoNodeUnitStorage) -> None:
        self.drop()
        self.__storage = dto.get_storage()
