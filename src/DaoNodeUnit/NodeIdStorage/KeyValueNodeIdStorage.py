...
class KeyValueNodeIdStorage(KeyValueCRUDInterface):
    def __init__(self, storage_name: str, critical_dump_flag: bool = True):
        """
        KeyValueStringStorage is an In memory storage based on KeyValueCRUDInterface from LibByzaticCommon.CRUD
        ...
        """
        self.__logger: logging.Logger = logging.getLogger("DaoNodeUnit-NodeIdStorage-logger")
        self.__storage: dict = {}
        self.__storage_name: str = storage_name
        self.__critical_dump_flag: bool = critical_dump_flag

    def create(self, key: str, value: list[str]) -> None:
        """
        Create string value in storage by key
        @param key: string identification parameter
        @param value: string value parameter
        @return: None if success or raise Exceptions.OperationIncompleteException
        """
        try:
            ops_name: str = str(inspect.currentframe().f_code.co_name)
            ...
            if key not in self.__storage:
                ...
                raise Exceptions.OperationIncompleteException(f"storage: {self.__storage_name} "
                                                              f"operation: {ops_name} "
    ...
    def read(self, key: str) -> list[str]:
        ...
        @param key: string identification parameter
        @return: str if success or raise Exceptions.OperationIncompleteException
        """
        try:
            ops_name: str = str(inspect.currentframe().f_code.co_name)
            self.__logger.debug(f"storage: {self.__storage_name} "
            ...
            if key in self.__storage:
                ...
                raise Exceptions.OperationIncompleteException(f"storage: {self.__storage_name} "
                                                              f"operation: {ops_name} "
                                                              f"msg: Can't get value by key {key} "
                                                              f"because no such key in storage")
        except Exceptions.OperationIncompleteException as oie:
            raise Exceptions.OperationIncompleteException(oie.args, errno=oie.errno)
        except Exception as err:
            raise Exceptions.OperationIncompleteException(err.args)

    def update(self, key: str, value: list[str]) -> int:
        """
        Update string value in storage by key
        @param key: string identification parameter
        @param value: string value parameter
        @return: 0 if success or raise Exceptions.OperationIncompleteException
        """
        try:
            ops_name: str = str(inspect.currentframe().f_code.co_name)
            deep_value: list[str] = self.__local_deepcopy(value, ops_name)
            self.__logger.debug(f"storage: {self.__storage_name} "
                                f"operation: {ops_name} "
    ...
    def read_list_values(self) -> list[list[str]]:
        """
        Read all value and keys in storage
        @return: dict if success or raise Exceptions.OperationIncompleteException
        """
        try:
            ops_name: str = str(inspect.currentframe().f_code.co_name)
            list_item: list[list[str]] = []
            for item in self.__storage:
                list_item.append(item)
            storage_object: list[list[str]] = self.__local_deepcopy(list_item, ops_name)
            self.__logger.debug(f"storage: {self.__storage_name} "
                                f"operation: {ops_name} "
    ...
    def read_list_keys(self) -> list[str]:
        """
        Read list of all keys in storage
        @return: list if success or raise Exceptions.OperationIncompleteException
        """
        try:
            ops_name: str = str(inspect.currentframe().f_code.co_name)
            list_items: list[str] = []
    ...
    def contains(self, key: str) -> bool:
        ...
        @return: bool if success or raise Exceptions.OperationIncompleteException
        """
        try:
            ops_name: str = str(inspect.currentframe().f_code.co_name)
            if key in self.__storage:
    ...
    def __local_deepcopy(self, value: Union[list[str], list[list[str]]], ops_name: str) -> Union[list[str], list[list[str]]]:
        try:
            self.__logger.debug(f"storage: {self.__storage_name} "
                                f"operation: {ops_name} "
                                f"msg: Running deepcopy for {value}")
            deepcopy_value: list[str] = deepcopy(value)
            self.__logger.debug(f"storage: {self.__storage_name} "
                                f"operation: {ops_name} "
    ...
    def __check_if_value_empty(self, value: list[str], ops_name: str) -> None:
        try:
            if value is None:
                self.__logger.debug(f"storage: {self.__storage_name} "
                                    f"operation: {ops_name} "
